...
- **Course chips**: The course code opens details; two explicit actions move or remove the course. Move reveals a native term selector that omits co-op terms and disables the current destination. Finder rows expose Add and Details. These commands use the same undoable store operations as drag and drop, which remains the spatial shortcut. The title and credits keep a separate fixed row, and actions never overlay course identity. Pointer drag keeps the anchored spring and velocity tilt; reduced-motion users receive the anchored position immediately.
