...
- **Calendar alignment**: `/tools/schedule` and `/pulse/schedule` use Calendar’s 24px desktop padding, plain 288px contextual aside, 24px region gap, and flexible data canvas. The aside has no background, border, radius, shadow, or shared scrolling. Route content owns its scroll regions. The timetable owns the sole canvas boundary: a 10px `border-subtle` frame with a 2px gutter and 8px surface fields.
...
- **Block anatomy**: Planner blocks show only course code and `section · type`; meetings below the tall threshold place all three on one line without changing time geometry. Sharer blocks show course and component because Workday data carries no section identifier. Full title, time, location, status, and people remain in accessible labels or read-only details. Avatar footers appear only on tall blocks. Blocks retain the documented course-color edge and surface mix; conflicts use the error ring.
...
- **Typography**: Schedule titles use the 20px title step; section headings and buttons use 14px; helper copy uses 13px; labels and metadata use 12px. Commit Mono appears only on course codes, section identifiers, times, rooms, and counts. Schedule controls use no tracked uppercase labels and no text below 12px.
