...
Directions uses one aligned route grid: a 44px Back column, a centered origin-to-destination marker track, and two equal inset From and To fields separated by 8px. The grid stays sticky inside the directions view's sole vertical scroll owner, so wheel or touch gestures begun over either the editor or results move the same bounded list. Results use flat divided rows with a visible selected edge and thin scrollbar. Choosing a result commits it, resets scroll, removes the list immediately, and starts a route when both endpoints exist. Loading, retry, and route summaries replace the list without retaining search results.
