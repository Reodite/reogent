...
- **Compact presentation**: Below 40rem, Outline opens first and provides 44px course and prerequisite-choice actions. Map remains available as the spatial view. At wider sizes the map opens directly, keeps a readable zoom floor, and pans instead of shrinking every node into one frame. React Flow edges and outer node wrappers stay out of the tab order; explicit node controls own keyboard access.
