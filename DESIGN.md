...
- **Course chips**: A fixed top row holds the course code and two 28px actions; a fixed bottom row holds the full title zone and credits. Actions never overlay or mask the course identity. On pointer drag, the overlay starts at the picked-up position, then its top-center springs under the cursor; velocity tilt pivots from that fixed top anchor. Reduced-motion users receive the anchored position immediately.
