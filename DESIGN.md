...
- **Composition**: Operate-mode workbench with 24px desktop padding, 16px region gaps, and 8px internal gaps. The top bar carries title, visible-label program selectors, then labeled actions. The body places a fixed 320px Requirements and Find Courses rail to the left of the horizontally scrollable year board.
- **Rail**: Requirements and Find Courses are separate `.neu-panel bg-surface rounded-2xl` cards in equal grid rows. Both use `min-w-0 min-h-0`, stable scrollbar gutters, and internal scrolling; neither card shrinks to make room for the other. Find Courses lists only courses not already planned. Its source row hides during drag, a successful drop removes it from the results, and lookup drags never animate back to the source.
