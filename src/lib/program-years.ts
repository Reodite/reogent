...
const DEGREE_TOTAL_RE = /(credits?\s+for\s+(the\s+)?degree|minimum\s+credits|overall\b.*\btotal\s+credits)/i;
...
function extractCodeMentions(label: string): CodeMention[] {
  ...
  for (const match of label.matchAll(TOKEN_RE)) {
    ...
    if (/^\d/.test(tok)) {
      ...
      // A non-Vancouver campus suffix ("HES_O") makes this subject out of
      // scope: clear it so the bare number that follows is dropped rather
      // than re-homed onto a Vancouver code.
...
function requirementGroups(label: string): string[][] {
  ...
  if (WHOLE_ROW_CHOICE_RE.test(label) || (branchLabels != null && branchLabels.length >= 2)) {
    return [allCodes];
  }
...
function mergeRequirement(base: YearRequirement, next: YearRequirement): YearRequirement {
...
export function parseProgramYears(text: string): ParsedProgramYears {
  ...
  for (const line of lines) {
    ...
    pending = pending && CONTINUATION_RE.test(line) ? mergeRequirement(pending, req) : req;
  }
...
export function hasYearRequirements(parsed: ParsedProgramYears): boolean {
  return parsed.years.some((y) => y.items.some((it) => it.kind === "course"));
}
...
export function requirementKey(programUrl: string, yearLabel: string, item: YearRequirement): string {
