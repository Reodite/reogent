...
// before pushing.
type KwToken = { type: "ONE_OF" } | { type: "ALL_OF" } | { type: "EITHER" } | { type: "AND" } | { type: "OR" };
  ...
  while (i < input.length) {
    ...
    if (atWordStart) {
      ...
      if (textBuf === "") {
        ...
        if (lastCodeSubject && (lastTokenType === "COMMA" || lastTokenType === "OR" || lastTokenType === "AND")) {
...
class Parser {
  ...
  parse(): Expr | null {
    ...
    const expr = this.tokens[0]?.type === "LABEL" && this.hasInterBranchOr() ? this.eitherTail() : this.andExpr();
      ...
      if (rest) {
        ...
        if (expr.kind === "and") {
          ...
          while (
            ...
            const a = children[children.length - 1] as Extract<Expr, { kind: "literal" }>;
  ...
  private andExpr(): Expr | null {
    ...
    while (this.isAndJoiner()) {
      ...
      if (sawSentenceBoundary && this.match("OR")) {
        this.consume();
        const lhs: Expr | null =
          children.length === 0 ? null : children.length === 1 ? children[0] : { kind: "and", children };
  ...
  private orExpr(): Expr | null {
    ...
    if (children.length === 0) return null;
    return children.length === 1 ? children[0] : { kind: "or", ui: "dropdown", children };
  }
  ...
  private codeList(labeled: boolean): Expr[] {
    ...
    while (true) {
      ...
      if (this.match("COMMA")) {
        ...
        let off = 1;
        while (this.peek(off)?.type === "AND" || this.peek(off)?.type === "OR") {
          off++;
        }
      ...
      } else if (labeled && this.match("OR") && this.peek(1)?.type === "LABEL") {
        this.consume();
        consumedSep = true;
      }
  ...
  private eitherBranch(labeled: boolean): { expr: Expr | null; hasLabel: boolean } {
    let hasLabel = false;
    while (this.match("LABEL")) {
      this.consume();
      hasLabel = true;
...
function groupBranchChildren(children: Expr[], joiners: Array<"or" | "and" | null>): Expr | null {
  const groups: Expr[][] = [];
  ...
  const conjuncts: Expr[] = groups.map((g) => (g.length === 1 ? g[0] : { kind: "or", ui: "dropdown", children: g }));
  return conjuncts.length === 1 ? conjuncts[0] : { kind: "and", children: conjuncts };
}
...
function normalize(expr: Expr): Expr {
  ...
  const children = expr.children.map(normalize).flatMap((c) => {
    if (c.kind === expr.kind && (c.kind !== "or" || (expr.kind === "or" && c.ui === expr.ui))) {
      return c.children;
    }
    return [c];
  });
...
const RECOMMEND_TAIL_RE = /\s+(?:(?:is|are)\s+)?(?:(?:strongly|highly|also)\s+)?recommended\b.*$/i;
...
export function missingPrereqs(expr: Expr | null, completed: Set<string>): string[] {
  ...
  switch (expr.kind) {
    case "code":
      return [expr.code];
    case "and":
      return expr.children.filter((c) => !isUnevaluable(c)).flatMap((c) => missingPrereqs(c, completed));
    case "or":
...
// Re-export the walker API alongside the donor parser so callers import from one module.
export { walkCodeLeaves, MAX_DEPTH } from "./walk";
export type { CodeLeafWithParent } from "./walk";
