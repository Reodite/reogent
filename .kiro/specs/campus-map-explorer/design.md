...
- Keep one fixed search field in discovery, place an icon-only Back action immediately before the Directions panel heading, and reserve the raised route card for two equal shared `TextInput` compact controls after a 16px marker track with a 12px gutter.
