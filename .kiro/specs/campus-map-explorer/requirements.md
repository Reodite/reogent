...
5. THE Building_Rail SHALL display Source_Provenance only for datasets that supply at least one field or photo rendered in the current building details.
...
4. WHEN both endpoint buildings are valid and the routing method is network, THE Map_Canvas SHALL display visible route fragments at full opacity with a casing, building-occluded fragments as one 30%-opacity primary stroke, opaque buildings with complete mutual depth occlusion, and basemap labels above every route pass; THE Building_Rail or compact sheet handle SHALL display distance and estimated walking time without another View route action.
